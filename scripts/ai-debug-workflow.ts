...
// 支持作为子模块使用：自动检测项目根目录
// 1. 首先尝试从 scripts/ 向上查找（本项目使用）
// 2. 然后尝试从可能的子模块路径查找（父项目使用）
const scriptDir = __dirname;
const possibleProjectRoots = [
  path.resolve(scriptDir, '..'),        // 从 scripts/ 到项目根（本项目）
  path.resolve(scriptDir, '../..'),      // 从父项目的子模块路径（如 parent/GithubActionAISelfBuilder/scripts/）
...
async function main() {
  ...
  const runIdFile = path.join(projectRoot, '.github', '.github_run_id.txt');
